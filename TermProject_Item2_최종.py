...
def run_list(root_node):
    ...
    #op 코드가 리스트로 되있는 경우는 lambda이므로 한번 더 value로 들어간다.
...
def run_func(op_code_node):
    ...
    def plus(node):
        ...
        l_node = run_expr(l_node)
        r_node = run_expr(r_node)
        while l_node.type is TokenType.ID:
            l_node = lookupTable(l_node.value)
        while r_node.type is TokenType.ID:
    ...
    def run_cond(node):
        ...
        while node is not None:
            l_node = node.value
            if run_expr(l_node).type is TokenType.TRUE:
    ...
    def define(node):
        l_node = node.value.next
        r_node = l_node.next
        # key 값으로 전달되는 것은 문자여야해서 value 로한다.
        # 만약 .value 를 안붙여 노드가 key 로 들어가면 나중에 문제가 된다.
        key = l_node.value
        r_node = run_expr(r_node)
        n_len = len(nested_key_dict)
        if n_len > 0:
            mother_dict[key] = nested_key_list[n_len-1]
        # qoute 일때는 isalpha()가 실행이 안된다.
    ...
    def run_lambda(node):
        # lambda 와 define lambda 일 때 괄호의 갯수를 맞추는 작업.
        if node.value.type is TokenType.LIST:
            l_node = node.value  # (lambda (formal para) (expression) ) actual para
        else:
            l_node = node        # (lambda (formal para) (expression) ) actual para

        formal_para = l_node.value.next.value
        actual_para = l_node.next  # actual para
        expression = l_node.value.next.next

        # nested 구현
        while expression.value.type is TokenType.DEFINE:
            nested_key = expression.value.next.value
            nested_key_list.append(nested_key)
            nested_key_dict[nested_key] = expression.value.next.next
            expression = expression.next

        l_key = formal_para
        l_value = actual_para
        # actual para가 keyword인 경우
        if l_value is not None and l_value.type is TokenType.LIST:
            l_value = run_expr(actual_para)

        # actual para 가 없을때 그대로 return
        if actual_para is None:

            return l_node

        if l_key.value is l_value.value:
            l_value = lookupTable(l_key.value)

        # 바인딩
        while l_key is not None:
            insertTable(l_key.value, l_value)
            l_key = l_key.next
            l_value = l_value.next

        # expression 수행 결과 저장
        result = run_expr(expression)
        # expression 수행 후 테이블에서 nested 함수
        while len(nested_key_list) is not 0:
            remove_list = nested_key_list.pop()
            if remove_list is not None:
                if lookupTable(remove_list) is not None:
                    del MyDiction[remove_list]
        # 테이블에서 제거
        remove_key = formal_para
        while remove_key is not None:
            # 이미 제거한 값을 또 지우려하면 에러.
            if lookupTable(remove_key.value) is None:
                remove_key = remove_key.next
            else:
                if len(MyDiction[remove_key.value]) > 1:
                    s = MyDiction[remove_key.value]
                    MyDiction[remove_key.value].pop()
                else:
                    del MyDiction[remove_key.value]
                remove_key = remove_key.next
        return result
...
def run_expr(root_node):
    ...
    if root_node.type is TokenType.ID:
        test = lookupTable(root_node.value)
        if test is not None:
            return test
        return root_node
        ...
        return root_node
    # 정의된 function을 호출한다.
    elif root_node.value.type is TokenType.ID:
        check_mother_list(root_node.value.value)
        expr = lookupTable(root_node.value.value)
        if expr is not None:
            expr.next = root_node.value.next
            return run_expr(Node(TokenType.LIST, expr))
        print '정의되지 않은 function 입니다.',
...
def check_mother_list(key):
    if key in mother_dict:
        nested_key = mother_dict[key]
        nested_lambda = nested_key_dict[nested_key]
        insertTable(nested_key, nested_lambda)
...
def insertTable(key, value):
    temp = MyDiction
    if lookupTable(key) is not None: # 변수의 scope를 위해 list로 저장
        recursive = [lookupTable(key)] + [value]
        del MyDiction[key]
    else:
        recursive = [value]
    MyDiction[key] = recursive


def lookupTable(key):
    temp = MyDiction
    if key not in MyDiction:
        return None
    return MyDiction[key][-1]
